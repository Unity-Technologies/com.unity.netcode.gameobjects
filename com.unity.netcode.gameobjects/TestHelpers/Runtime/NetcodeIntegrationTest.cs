...
namespace Unity.Netcode.TestHelpers.Runtime
{
    ...
    public abstract class NetcodeIntegrationTest
    {
        ...
        /// <summary>
        /// Override this method and return false in order to be able
        /// to test startup failure conditions
        /// </summary>
        protected virtual bool StartupFailureIsTestFailure()
        {
            return true;
        }
        ...
        protected IEnumerator StartServerAndClients()
        {
            if (CanStartServerAndClients())
            {
                ...
                bool serverStartSucceeded = m_ServerNetworkManager.State == NetworkManagerState.Ready;
                bool clientStartSucceeded = m_ClientNetworkManagers.Where((c) => c.State == NetworkManagerState.Inactive).Count() == 0;
                if (!serverStartSucceeded || !clientStartSucceeded)
                {
                    if (StartupFailureIsTestFailure())
                    {
                        Assert.Fail("One or more clients failed to start up and connect");
                    }

                    // The remaining code after this can't run if startup didn't finish,
                    // even if it's not a failure condition for this test.
                    yield break;
                }
        ...
        protected IEnumerator WaitForClientsConnectedOrTimeOut(NetworkManager[] clientsToCheck)
        {
            ...
            // logic here: Wait for everything to enter either the ready state OR the inactive state
            // Ready state == Startup has finished
            // Inactive state == Startup has failed
            yield return WaitForConditionOrTimeOut(() => clientsToCheck.Where((c) => c.State == NetworkManagerState.Ready || c.State == NetworkManagerState.Inactive).Count() == remoteClientCount &&
            (m_ServerNetworkManager.State == NetworkManagerState.Ready || m_ServerNetworkManager.State == NetworkManagerState.Inactive));
        }
