...
namespace Unity.Netcode.RuntimeTests.Transport
{
    public class SIPTransportTests
    {
        ...
        public void SendReceiveData()
        {
            ...
            serverEvent = server.PollEvent(out ulong newClientId, out NetworkDelivery _, out ArraySegment<byte> serverPayload, out _);
            clientEvent = client.PollEvent(out ulong newServerId, out NetworkDelivery _, out ArraySegment<byte> clientPayload, out _);
