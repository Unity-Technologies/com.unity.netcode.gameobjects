...
namespace Unity.Netcode
{
	internal struct CreateObjectMessage : INetworkMessage
	{
		...
		private const byte k_HasCreationMetadata = 0x08;
		...
		internal bool HasCustomSpawnData
		{
			get
			{
				return GetFlag(k_HasCreationMetadata);
			}
			set
			{
				SetFlag(value, k_HasCreationMetadata);
			}
		}
		...
		public void Serialize(FastBufferWriter writer, int targetVersion)
		{
			...
			if (HasCustomSpawnData)
			{
				BytePacker.WriteValuePacked(writer, CustomSpawnData.Length);
				foreach (var mdByte in CustomSpawnData)
				{
					BytePacker.WriteValuePacked(writer, mdByte);
				}
			}
		...
		public bool Deserialize(FastBufferReader reader, ref NetworkContext context, int receivedMessageVersion)
		{
			...
			if (HasCustomSpawnData)
			{
				...
				CustomSpawnData = new byte[length];
				var clientId = (byte)0;
				for (int i = 0; i < length; i++)
				{
					ByteUnpacker.ReadValuePacked(reader, out clientId);
					CustomSpawnData[i] = clientId;
				}
			}
