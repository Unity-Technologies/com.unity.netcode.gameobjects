            ...
            foreach (ulong clientId in m_TargetList)
            {
                ...
                if (k_SendDict[clientId].IsEmpty)
                {
                    ...
                    switch (queueItem.QueueItemType)
                    {
                        ...
                        case RpcQueueContainer.QueueItemType.ServerRpc:
                            k_SendDict[clientId].Writer.WriteByte(NetworkConstants.k_SERVER_RPC); // MessageType
                            break;
                        case RpcQueueContainer.QueueItemType.ClientRpc:
                            k_SendDict[clientId].Writer.WriteByte(NetworkConstants.k_CLIENT_RPC); // MessageType
                            break;
                    }
                }
                ...
                PerformanceDataManager.Increment(ProfilerConstants.k_NumberBytesSent, queueItem.MessageData.Count);
                PerformanceDataManager.Increment(ProfilerConstants.k_NumberOfRPCsSent);
            }
        }
        ...
        public void SendItems(int thresholdBytes, SendCallbackType sendCallback)
        {
            foreach (KeyValuePair<ulong, SendStream> entry in k_SendDict)
            {
                if (!entry.Value.IsEmpty)
                {
                    ...
                    int length = (int)k_SendDict[entry.Key].Stream.Length;
                    ...
                    if (length >= thresholdBytes)
                    {
                        ...
                        PerformanceDataManager.Increment(ProfilerConstants.k_NumberOfRPCBatchesSent);
                    }
                }
            }
        }
