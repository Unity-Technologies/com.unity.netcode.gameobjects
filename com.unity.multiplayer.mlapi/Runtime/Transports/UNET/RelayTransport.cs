...
namespace Unity.Netcode.Transports.UNET
{
    public static class RelayTransport
    {
        ...
        public static int Connect(int hostId, string serverAddress, int serverPort, int exceptionConnectionId, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.Connect(hostId, serverAddress, serverPort, exceptionConnectionId, out error);
            }
        ...
        public static int ConnectWithSimulator(int hostId, string serverAddress, int serverPort, int exceptionConnectionId, out byte error, ConnectionSimulatorConfig conf)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.ConnectWithSimulator(hostId, serverAddress, serverPort, exceptionConnectionId, out error, conf);
            }
        ...
        public static int ConnectEndPoint(int hostId, EndPoint endPoint, int exceptionConnectionId, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.ConnectEndPoint(hostId, endPoint, exceptionConnectionId, out error);
            }
        ...
        public static int AddHost(HostTopology topology, int port, bool createServer)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.AddHost(topology, port);
            }
        ...
        public static int AddHost(HostTopology topology, int port, string ip, bool createServer)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.AddHost(topology, port, ip);
            }
        ...
        public static int AddHostWithSimulator(HostTopology topology, int minTimeout, int maxTimeout, int port, string ip, bool createServer)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.AddHostWithSimulator(topology, minTimeout, maxTimeout);
            }
        ...
        public static int AddHostWithSimulator(HostTopology topology, int minTimeout, int maxTimeout, int port, bool createServer)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.AddHostWithSimulator(topology, minTimeout, maxTimeout, port);
            }
        ...
        public static int AddWebsocketHost(HostTopology topology, int port, bool createServer)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.AddWebsocketHost(topology, port);
            }
        ...
        public static int AddWebsocketHost(HostTopology topology, int port, string ip, bool createServer)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.AddWebsocketHost(topology, port, ip);
            }
        ...
        public static bool Send(int hostId, int connectionId, int channelId, byte[] buffer, int size, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.Send(hostId, connectionId, channelId, buffer, size, out error);
            }
            ...
            if (!s_IsClient)
            {
                ...
                for (byte i = 0; i < sizeof(ulong); i++)
                {
                    buffer[connectionIdOffset + i] = ((byte)((ulong)connectionId >> (i * 8)));
                }
            }
        ...
        public static bool QueueMessageForSending(int hostId, int connectionId, int channelId, byte[] buffer, int size, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.QueueMessageForSending(hostId, connectionId, channelId, buffer, size, out error);
            }
        ...
        public static bool SendQueuedMessages(int hostId, int connectionId, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.SendQueuedMessages(hostId, connectionId, out error);
            }
        ...
        public static NetworkEventType ReceiveFromHost(int hostId, out int connectionId, out int channelId, byte[] buffer, int bufferSize, out int receivedSize, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.ReceiveFromHost(hostId, out connectionId, out channelId, buffer, bufferSize, out receivedSize, out error);
            }
        ...
        public static NetworkEventType Receive(out int hostId, out int connectionId, out int channelId, byte[] buffer, int bufferSize, out int receivedSize, out byte error)
        {
            if (!Enabled)
            {
                return UnityEngine.Networking.NetworkTransport.Receive(out hostId, out connectionId, out channelId, buffer, bufferSize, out receivedSize, out error);
            }
                ...
                case NetworkEventType.ConnectEvent:
                    {
                        if (s_IsClient)
                        {
                            ...
                            {
                                buffer[i] = ipv6AddressBuffer[i];
                            }

                            for (byte i = 0; i < sizeof(ushort); i++)
                            {
                                buffer[16 + i] = ((byte)(s_Port >> (i * 8)));
                            }
