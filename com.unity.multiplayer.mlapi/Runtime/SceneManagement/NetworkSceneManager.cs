        ...
        internal void OnFirstSceneSwitchSync(uint sceneIndex, Guid switchSceneGuid)
        {
            ...
            using (PooledBitStream stream = PooledBitStream.Get())
            {
                using (PooledBitWriter writer = PooledBitWriter.Get(stream))
                {
                    ...
                    networkingManager.MessageSender.Send(networkingManager.ServerClientId, MLAPIConstants.MLAPI_CLIENT_SWITCH_SCENE_COMPLETED, Channel.Internal, stream, SecuritySendFlags.None);
                }
            }
            ...
            for (int j = 0; j < networkingManager.ConnectedClientsList.Count; j++)
            {
                if (networkingManager.ConnectedClientsList[j].ClientId != networkingManager.ServerClientId)
                {
                    using (PooledBitStream stream = PooledBitStream.Get())
                    {
                        ...
                        networkingManager.MessageSender.Send(networkingManager.ConnectedClientsList[j].ClientId, MLAPIConstants.MLAPI_SWITCH_SCENE, Channel.Internal, stream, SecuritySendFlags.None);
                    }
                }
            }
            ...
            using (PooledBitStream stream = PooledBitStream.Get())
            {
                using (PooledBitWriter writer = PooledBitWriter.Get(stream))
                {
                    ...
                    networkingManager.MessageSender.Send(networkingManager.ServerClientId,
                        MLAPIConstants.MLAPI_CLIENT_SWITCH_SCENE_COMPLETED, Channel.Internal, stream, SecuritySendFlags.None);
                }
            }
