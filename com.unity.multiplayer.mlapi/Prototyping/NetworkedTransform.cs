...
namespace MLAPI.Prototyping
{
    ...
    public class NetworkedTransform : NetworkedBehaviour
    {
        ...
        private void Update()
        {
            if (IsOwner)
            {
                if (NetManager.NetworkTime - lastSendTime >= (1f / FixedSendsPerSecond) && (Vector3.Distance(transform.position, lastSentPos) > MinMeters || Quaternion.Angle(transform.rotation, lastSentRot) > MinDegrees))
                {
                    lastSendTime = NetManager.NetworkTime;
                    lastSentPos = transform.position;
                    lastSentRot = transform.rotation;
                    ...
                    if (IsServer)
                    {
                        ApplyTransformClientRpc(transform.position, transform.rotation.eulerAngles,
                            new ClientRpcParams {Send = new ClientRpcSendParams {TargetClientIds = NetManager.ConnectedClientsList.Where(c => c.ClientId != OwnerClientId).Select(c => c.ClientId).ToArray()}});
                    }
                    else
                    {
            ...
            else
            {
                ...
                if ((IsServer && InterpolateServer && InterpolatePosition) || (!IsServer && InterpolatePosition))
                {
                    ...
                    float sendDelay = (IsServer || !EnableRange || !AssumeSyncedSends || NetManager.ConnectedClients[NetManager.LocalClientId].PlayerObject == null) ? (1f / FixedSendsPerSecond) : GetTimeForLerp(transform.position, NetManager.ConnectedClients[NetManager.LocalClientId].PlayerObject.transform.position);
        ...
        private void SubmitTransformServerRpc(Vector3 position, Vector3 eulerAngles, ServerRpcParams rpcParams = default)
        {
            ...
            if (EnableRange)
            {
                ...
                {
                    if (!clientSendInfo.ContainsKey(NetManager.ConnectedClientsList[i].ClientId))
                    {
                        clientSendInfo.Add(NetManager.ConnectedClientsList[i].ClientId, new ClientSendInfo()
                        {
                    ...
                    ClientSendInfo info = clientSendInfo[NetManager.ConnectedClientsList[i].ClientId];
                    Vector3? receiverPosition = NetManager.ConnectedClientsList[i].PlayerObject == null ? null : new Vector3?(NetManager.ConnectedClientsList[i].PlayerObject.transform.position);
                    Vector3? senderPosition = NetManager.ConnectedClients[OwnerClientId].PlayerObject == null ? null : new Vector3?(NetManager.ConnectedClients[OwnerClientId].PlayerObject.transform.position);

                    if ((receiverPosition == null || senderPosition == null && NetManager.NetworkTime - info.lastSent >= (1f / FixedSendsPerSecond)) || NetManager.NetworkTime - info.lastSent >= GetTimeForLerp(receiverPosition.Value, senderPosition.Value))
                    {
                        info.lastSent = NetManager.NetworkTime;
                        info.lastMissedPosition = null;
                        info.lastMissedRotation = null;

                        ApplyTransformClientRpc(position, eulerAngles,
                            new ClientRpcParams {Send = new ClientRpcSendParams {TargetClientIds = new[] {NetManager.ConnectedClientsList[i].ClientId}}});
                    }
                    else
                    {
        ...
        private void CheckForMissedSends()
        {
            for (int i = 0; i < NetManager.ConnectedClientsList.Count; i++)
            {
                ...
                if ((receiverPosition == null || senderPosition == null && NetManager.NetworkTime - info.lastSent >= (1f / FixedSendsPerSecond)) || NetManager.NetworkTime - info.lastSent >= GetTimeForLerp(receiverPosition.Value, senderPosition.Value))
                {
                    /* why is this??? ->*/Vector3? pos = NetManager.ConnectedClients[OwnerClientId].PlayerObject == null ? null : new Vector3?(NetManager.ConnectedClients[OwnerClientId].PlayerObject.transform.position);
                    /* why is this??? ->*/Vector3? rot = NetManager.ConnectedClients[OwnerClientId].PlayerObject == null ? null : new Vector3?(NetManager.ConnectedClients[OwnerClientId].PlayerObject.transform.rotation.eulerAngles);
