...
/// <summary>
/// Colors the player prefab instance based on client indentifier
/// </summary>
public class PlayerColor : NetworkBehaviour
{
    ...
    public void SetObjectColor(NetworkObject objectToColor)
    {
        var meshRenderer = objectToColor.GetComponent<MeshRenderer>();
        if (meshRenderer)
        {
            ulong colorIndex = objectToColor.OwnerClientId - (ulong)(NetworkManager.DistributedAuthorityMode && NetworkManager.CMBServiceConnection ? 1 : 0);
            var objectColor = s_Colors[colorIndex % Convert.ToUInt64(s_Colors.Length)];
