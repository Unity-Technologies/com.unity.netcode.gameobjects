...
public class SceneLoader : MonoBehaviour
{
    ...
    {
        if (!m_NetworkManager.IsServer || !m_NetworkManager.IsListening) 
        {
            return;
        }

        var nextSceneName = GetNextSceneNameToLoad();
        var loadStatus = m_NetworkManager.SceneManager.LoadScene(nextSceneName, LoadSceneMode.Single);
        if (loadStatus != SceneEventProgressStatus.Started)
        {
            m_NetworkManager.LogMessage($"Failed to start scene loading event for {nextSceneName}! [Status: {loadStatus}]");
        }
        else
        {
