...
namespace TestProject.RuntimeTests
{
    ...
    public class SceneObjectsNotDestroyedOnShutdownTest : NetcodeIntegrationTest
    {
        ...
        public IEnumerator SceneObjectsNotDestroyedOnShutdown()
        {
            ...
            var loadedInSceneObject = FindObjects.FindObjectsByType<NetworkObject>().Where((c) => c.name.Contains(k_SceneObjectName)).FirstOrDefault();
            ...
            var loadedInSceneObjects = FindObjects.FindObjectsByType<NetworkObject>().Where((c) => c.name.Contains(k_SceneObjectName));
            Assert.IsTrue(loadedInSceneObjects.Count() > 1, $"Only found one instance of {k_SceneObjectName} after client connected!");
            lateJoin.Shutdown();
            yield return m_DefaultWaitForTick;
            loadedInSceneObjects = FindObjects.FindObjectsByType<NetworkObject>().Where((c) => c.name.Contains(k_SceneObjectName));
        ...
        public IEnumerator ChildSceneObjectsDoNotDestroyOnShutdown()
        {
            ...
            loadedInSceneObjects = FindObjects.FindObjectsByType<NetworkObject>().Where(o => o.name.Contains(k_SceneObjectName)).ToArray();
