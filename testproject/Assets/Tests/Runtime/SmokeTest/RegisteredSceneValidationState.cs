    ...
    public class RegisteredSceneValidationState : SceneAwareSmokeTestState
    {
        ...
        protected override bool OnProcessState()
        {
            ...
            foreach (var sceneGroup in m_SceneReferenced)
            {
                ...
                {
                    Assert.That(sceneNamesInBuildSettings.Contains(sceneName), $"Could not find {sceneName} within the {nameof(sceneNamesInBuildSettings)} listing! {nameof(sceneNamesInBuildSettings)} contains {sceneNamesInBuildSettings.Count} scene names.");
                }
            }
